...
import OurProdcutSwiper from "@/components/front/detail-product/our-product";
import ProductImage from "@/components/front/detail-product/product-image";
import { Product } from "@/types";
import ProductActions from "@/components/front/detail-product/product-actions";
...
export default async function Page({ params }: { params: { id: string } }) {
  ...
  // Compare the string param 'id' with the product's id converted to a string.
  const product = products.find((product: Product) => String(product.id) === params.id);
  if (!product) {
    return (
      <>
        <h1>Not Found!</h1>
  ...
  return (
    <>
      ...
      {/* OUR PRODUCT */}
      <OurProdcutSwiper products={allProducts} />
    </>
  )
}