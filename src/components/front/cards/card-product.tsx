...
export default function CardProduct({ name, image, slug, category, colors, price, isSwiper }: CardProductProps) {
  return (
    <article className={clsx("w-full group", { 'swiper-slide': isSwiper })}>
      <Link href={`/product/${slug || name}`} className="h-[300px] sm:h-[340px] md:h-[380px] lg:h-[420px] xl:h-[500px] w-full relative block rounded-[20px] md:rounded-[24px] lg:rounded-[28px] xl:rounded-[32px] overflow-hidden">
        <Image className="h-full w-full object-cover transition-all transform duration-200 group-hover:scale-110" src={`/images/${image}`} height={488} width={432} alt={name} />
